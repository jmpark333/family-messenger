...
interface ChatStore {
  ...
  processedMessageIds: Set<string>;  // 중복 방지용: 처리한 메시지 ID 저장
  ...
  // 피어 발견
  addDiscoveredPeer: (userId: string, peer: DiscoveredPeer) => void;
  removeDiscoveredPeer: (userId: string) => void;
  updateDiscoveredPeer: (userId: string, updates: Partial<DiscoveredPeer>) => void;
  clearDiscoveredPeers: () => void;
  setPeerDiscoveryEnabled: (enabled: boolean) => void;
...
export const useChatStore = create<ChatStore>((set, get) => ({
  ...
  // 피어 발견 액션
  addDiscoveredPeer: (userId, peer) =>
    set((state) => {
      const newDiscoveredPeers = new Map(state.discoveredPeers);
      newDiscoveredPeers.set(userId, peer);
      return { discoveredPeers: newDiscoveredPeers };
    }),

  removeDiscoveredPeer: (userId) =>
    set((state) => {
      const newDiscoveredPeers = new Map(state.discoveredPeers);
      newDiscoveredPeers.delete(userId);
      return { discoveredPeers: newDiscoveredPeers };
    }),

  updateDiscoveredPeer: (userId, updates) =>
    set((state) => {
      const newDiscoveredPeers = new Map(state.discoveredPeers);
      const peer = newDiscoveredPeers.get(userId);
      if (peer) {
        newDiscoveredPeers.set(userId, { ...peer, ...updates });
      }
      return { discoveredPeers: newDiscoveredPeers };
    }),

  clearDiscoveredPeers: () =>
    set({ discoveredPeers: new Map() }),

  setPeerDiscoveryEnabled: (enabled) =>
    set({ isPeerDiscoveryEnabled: enabled }),
  ...
  saveMessage: async (message) => {
    try {
      ...
      // 중복 방지: 이미 처리된 메시지인지 확인
      set((state) => {
        if (state.processedMessageIds.has(message.id)) {
          console.log('[ChatStore] Message already processed, skipping:', message.id);
          return state;
        }
        return {
          messages: [...state.messages, message],
