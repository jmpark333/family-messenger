...
export class P2PManager {
  ...
  private setupConnectionEvents(conn: DataConnection, isIncoming: boolean = false) {
    ...
    // 연결됨
    conn.on('open', async () => {
      console.log('[P2P] Connection opened:', peerId, 'isIncoming:', isIncoming);
      console.log('[P2P] Before storing - connection peerId:', conn.peer, 'metadata:', conn.metadata, 'reliable:', conn.options?.reliable);

      // Ensure myPeerId is set in store immediately (fixes timing issue)
      const currentMyPeerId = useChatStore.getState().myPeerId;
      if (!currentMyPeerId) {
        const myPeerId = this.getMyPeerId();
        if (myPeerId) {
          console.log('[P2P] Setting myPeerId in store on connection open:', myPeerId);
          useChatStore.getState().setMyInfo(myPeerId, '나');
        }
      }

      try {
        // PIN 검증 수행
        console.log('[P2P] Starting PIN verification...');
        const verified = await this.performPinVerification(conn, peerId, isIncoming);
        console.log('[P2P] PIN verification result:', verified);
        ...
        console.error('[P2P] Error during PIN verification:', error);
        conn.close();
        return;
      }

      this.connections.set(peerId, conn);
      console.log('[P2P] Connection stored in map:', peerId, 'total connections:', this.connections.size, 'keys:', Array.from(this.connections.keys()));
  ...
  private handleIncomingData(peerId: string, data: any) {
    try {
      ...
      switch (message.type) {
        ...
        // PIN 검증 우회로 auth 메시지는 더 이상 처리하지 않음
        // case 'auth-challenge':
   ...
   * NOTE: 현재는 PIN 검증을 우회하고 항상 true를 반환합니다.
   * TODO: 추후 공유 PIN(shared family PIN) 또는 공개키 기반 인증으로 재구현 필요
   */
  private async performPinVerification(
    ...
    console.log('[P2P] performPinVerification called, isIncoming:', isIncoming);
    console.log('[P2P] PIN verification bypassed - auto-approving connection');

    // PIN 검증 우회: 항상 true 반환
    // WebRTC의 DLS-SRTP 암호화가 메시지 보안을 담당합니다
    return true;

    /* 아래는 기존 PIN 검증 로직 (주석 처리됨)
    const myPin = useChatStore.getState().additionalPin;
    console.log('[P2P] myPin:', myPin ? 'set' : 'not set');
    if (!myPin) {
    ...
    if (isIncoming) {
      console.log('[P2P] Incoming connection - waiting for auth-challenge');
      ...
      console.log('[P2P] Outgoing connection - sending auth-challenge');
      // 발신 연결: 챌린지 전송 후 응답 대기
      const challenge = await createAuthChallenge();
      console.log('[P2P] Challenge created:', challenge);
  ...
  connectToPeer(peerId: string): Promise<void> {
    return new Promise(async (resolve, reject) => {
      ...
      // 연결 대기
      conn.on('open', () => {
        console.log('[P2P] connectToPeer - connection opened for:', peerId, 'stored in map:', this.connections.has(peerId));
        resolve();
      });
  ...
  sendToPeer(peerId: string, message: DataMessage): boolean {
    ...
    console.log('[P2P] Current connections in map:', Array.from(this.connections.keys()));

    // Check for ID mismatch between store and connections map
    const store = useChatStore.getState();
    console.log('[P2P] Peers in store:', Array.from(store.peers.keys()));
    console.log('[P2P] Peer IDs in store values:', Array.from(store.peers.values()).map(p => p.id));
    console.log('[P2P] Looking for peerId in connections:', peerId, 'exists:', this.connections.has(peerId));
    if (!conn && this.connections.size > 0) {
      console.log('[P2P] Connection not found! Checking for potential ID mismatch...');
      for (const [key, connection] of this.connections.entries()) {
        console.log('[P2P] Connection entry - key:', key, 'conn.peer:', connection.peer, 'conn.open:', connection.open);
      }
    }

    if (!conn || !conn.open) {
      console.error('[P2P] No connection to peer:', peerId, '- conn exists:', !!conn, 'conn.open:', conn?.open);
      console.error('[P2P] All connection states:', Array.from(this.connections.entries()).map(([id, c]) => [id, c.open, c.peer, c.connectionId]));
      return false;
    }
  ...
  isConnectedTo(peerId: string): boolean {
    const conn = this.connections.get(peerId);
    console.log('[P2P] isConnectedTo - peerId:', peerId, 'conn exists:', !!conn, 'open:', conn?.open);
    if (!conn) {
      console.log('[P2P] isConnectedTo - Current connection keys:', Array.from(this.connections.keys()));
    }
...
export function initP2PManager(config: P2PConfig, events: P2PEvents): P2PManager | null {
  // Check if additional PIN is available before initializing
  const additionalPin = useChatStore.getState().additionalPin;
  if (!additionalPin) {
    console.warn('[P2P] Cannot initialize P2PManager yet: additional PIN not set, waiting...');
    return null;
  }
  ...
  console.log('[P2P] Initializing P2PManager with additionalPin:', additionalPin);
